...
# CaDiCaL, GTest, LibBF and minisat are pinned to a tag or commit inside
# their scripts, which the script hash already covers -- OutputCheck, a
# test-only dependency that is never linked into anything, is the one that
# is not.
#
