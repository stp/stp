; Compare the native fp.to_ubv circuit with an independent SymFPU encoding
; over every value of (3,4). The oracle converts the same float expressed
...
(declare-fun native-conv () (_ BitVec 5))
(declare-fun oracle-conv () (_ BitVec 5))

(assert (fp.isZero minus-zero))
(assert (fp.isNegative minus-zero))
(assert (= native-conv ((_ fp.to_ubv 5) rm a)))
(assert (= oracle-conv ((_ fp.to_ubv 5) rm (fp.add RNE a minus-zero))))
