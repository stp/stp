...
;
; So the allowance is width/--bv-term-abstraction-value-divisor now, floored
; at one and capped by --bv-term-abstraction-rounds, which stays a ceiling and
; keeps every spelling it had. This multiply is 64 bits, so the four legs
; below ask for 64/8, the flat ceiling, 64/4, and a ceiling that bites before
; either.
;
...
;
; RUN: %solver --incremental=off -d -s --bv-eq-abstraction=1 --bv-term-abstraction=1 %s 2>&1 | %OutputCheck --check-prefix=SCALED %s
; RUN: %solver --incremental=off -d -s --bv-eq-abstraction=1 --bv-term-abstraction=1 --bv-term-abstraction-value-divisor=0 %s 2>&1 | %OutputCheck --check-prefix=FLAT %s
...
; SCALED: ^sat$
;
; Turning the scaling off restores the flat ceiling exactly, which is what
; makes the two configurations comparable at all: measuring whether the rate
; helps means being able to ask for the thing it replaced.
